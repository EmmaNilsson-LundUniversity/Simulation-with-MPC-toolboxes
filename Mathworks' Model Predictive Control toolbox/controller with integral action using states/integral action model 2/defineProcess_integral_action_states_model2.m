% % % Model of the quadruple tank process % % %
...
A = [A zeros(4,2)  zeros(4,2);   % redefining matrices to match the integral action model
        -[kc1 0 0 0; 
      0 kc2 0 0] zeros(2) eye(2);
        zeros(2,6) zeros(2)];
...
%                   Weigths
Weights.OV = [10 10 0 0 0.5 0.5] ;% Weights on y (Output signals)
Weights.MV = [0.5 0.5]; % Weigths on u (Input signals)
Weights.ManipulatedVariablesRate = [10 10]; % Weigths on delta u (slew rate)
...
%              Setting disturbance model to zero
setoutdist(mpcobj,'model',tf(zeros(6,1)));
