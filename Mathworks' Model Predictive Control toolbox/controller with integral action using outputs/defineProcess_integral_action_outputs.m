...
B = [B B]; %redefining B to match the integral action model
...
G = setmpcsignals(G,'MV',[1 2],'UD', [3 4], 'MO',[1 2]); %see README.md for information about this code

%                   Creating MPC-object
mpcobj = mpc(G,Ts,Hp,Hu, Weights,MV,OV);


% using the toolbox's disturbance models to set up integral action
Adist = zeros(2);
Bdist = 2*eye(2);
Cdist = eye(2);
Ddist = zeros(2);
distMod = ss(Adist, Bdist, Cdist, Ddist);
